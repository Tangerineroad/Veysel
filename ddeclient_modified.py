...
# 
# Serkan YILDIZ tarafından bazı bölümleri değiştirilmiştir. yildiz@serkan.net
# www.serkan.net
#-------------------------------------------------------------------------------
...
##        print(service)
##        print(topic)
##        self.connection = psycopg2.connect(database="imkb_db", user="postgres", password="HER67NAH", host="localhost", port=5432)
##        self.cursor = self.connection.cursor()
        ...
        if not self._hConv:
            print("===============================")
            print("DDE Bağlantısı sağlanamıyor...")
            print("===============================")
##            MessageBox(None, 'Dikkat!!! Sunucu bağlantısı sağlanamıyor...', 'Bist Veri Transferi', 0)
            raise SystemExit
    ...
    def _callback(self, wType, uFmt, hConv, hsz1, hsz2, hDdeData, dwData1, dwData2):
        ...
        if wType == XTYP_ADVDATA:  # value of the data item has changed [hsz1 = topic; hsz2 = item; hDdeData = data]
            ...
            if pData:
                ...
                item = create_string_buffer(('\000' * 256).encode())
                DDE.QueryString(self._idInst, hsz2, item, 256, CP_WINANSI)
        ...
        if wType == XTYP_DISCONNECT:
            MessageBox(None, 'Dikkat!!! DDE veri transferi sunucu tarafından durduruldu...', 'Bist Veri Transferi', 0)
