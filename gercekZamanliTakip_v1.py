...
def readSymbols():
    f = codecs.open("sabitler\\allSymbols.csv","r","utf8")
            ...
            for jj in range(ii+1,len(vadeler)):
                if cVade in [vadeler[ii],vadeler[jj]]:
                    ...
                    if item in hesaplar.keys():
                        hesaplar[item].append([[v1a,v1s,v1n],[v2a,v2s,v2n]])
                    else:
                        hesaplar[item] = [[[v1a,v1s,v1n],[v2a,v2s,v2n]]]
    ...
    while True:
        ...
        for item in allSyms:
            for each in hesaplar[item]:
                sa = ALL_VALUES[each[0][0]]
                ss = ALL_VALUES[each[0][1]]
                va = ALL_VALUES[each[1][0]]
                vs = ALL_VALUES[each[1][1]]
                ...
                if sa > 0 and ss > 0 and va > 0 and vs > 0:
                    ...
                    price = ' : {:8.2f}, {:8.2f}, {:8.2f}, {:8.2f} : {:8.4f}'.format(sa,ss,va,vs,getiri_norm)

                    ALL_PAIRS[pair] = [getiri_norm,price]

        ktb = sorted(ALL_PAIRS.items(), key=lambda kv: kv[1])
        btk = sorted(ALL_PAIRS.items(), key=lambda kv: kv[1], reverse=True)

        const1 = not [(x[0],x[1]) for x in ktb_old[:num_of_lines]] == [(x[0],x[1]) for x in ktb[:num_of_lines]]
        const2 = not [(x[0],x[1]) for x in btk_old[:num_of_lines]] == [(x[0],x[1]) for x in btk[:num_of_lines]]
        if const1 and const2:
            ...
            print("|----------------------------------------------------------------------------------------------------------------------------------------------------------------|")
            print("|            Largest to smallest                                                 |              Smallest to largest                                              |")
            print("|                                                                                |                                                                               |")
            print("|    SPOT         VADE       SPOT AF   SPOT SF   VADE AF   VADE SF     Getiri    |      SPOT         VADE       SPOT AF   SPOT SF   VADE AF   VADE SF     Getiri |")
            print("|----------------------------------------------------------------------------------------------------------------------------------------------------------------|")
            for ind in range(0,len(btk)):
                #if ind < num_of_lines:
                if ind < len(hisseler)/4:
                    print("| "+btk[ind][0] + btk[ind][1][1] + "    |   " + ktb[ind][0] + ktb[ind][1][1]+" |")
            print("|----------------------------------------------------------------------------------------------------------------------------------------------------------------|")
            first_time = True
        ...
        if not first_time:
            print ("\033[A                             \033[A")
        first_time = False
        print('Update interval: ' + str((d2-d1).microseconds/1000000000) + ' seconds')
